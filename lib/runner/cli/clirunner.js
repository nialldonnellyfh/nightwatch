...
CliRunner.prototype = {
  ...
  setupParallelMode : function(envs, done) {
    console.log('Parallel model set', envs);
  ...
  startChildProcesses : function(envs, doneCallback) {
    ...
    envs.forEach(function(item, index) {
      ...
      cliArgs.push('--env', item, '--parallel-mode', '--tag');
      var env = process.env;
  ...
  startTestWorkers : function(finishCallback) {
    ...
    function checkFilePush(basename, file){
      if(self.test_settings.tag_filter){
        return fileMatcher.tags.match(file, self.test_settings.tag_filter);
      }else { // only js files
